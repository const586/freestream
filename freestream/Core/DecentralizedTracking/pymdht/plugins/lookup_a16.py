...
class _LookupQueue(object):
    ...
    def bootstrap(self, rnodes, max_nodes):
        qnodes = [ _QueuedNode(n, n.id.log_distance(self.info_hash), None) for n in rnodes ]
    ...
    def on_response(self, src_node, nodes, token, max_nodes):
        qnode = _QueuedNode(src_node, src_node.id.log_distance(self.info_hash), token)
        self._add_responded_qnode(qnode)
        qnodes = [ _QueuedNode(n, n.id.log_distance(self.info_hash), None) for n in nodes ]
        self._add_queued_qnodes(qnodes)
        return self._pop_nodes_to_query(max_nodes)
    ...
    def get_closest_responded_qnodes(self, num_nodes = ANNOUNCE_REDUNDANCY):
    ...
    def _add_queued_qnodes(self, qnodes):
        for qnode in qnodes:
            if qnode.node.ip not in self.queued_ips and qnode.node.ip not in self.queried_ips:
...
class GetPeersLookup(object):
    ...
    def __init__(self, my_id, lookup_id, info_hash, callback_f, bt_port = 0):
        ...
        self._get_peers_msg = message.OutgoingGetPeersQuery(my_id, info_hash)
    ...
    def _get_max_nodes_to_query(self):
        if self._slow_down:
            return min(self.slowdown_alpha - self._num_parallel_queries, self.slowdown_m)
        return min(self.normal_alpha - self._num_parallel_queries, self.normal_m)

    def start(self, bootstrap_rnodes):
        self._running = True
        nodes_to_query = self._lookup_queue.bootstrap(bootstrap_rnodes, self.bootstrap_alpha)
    ...
    def on_response_received(self, response_msg, node_):
        ...
        nodes_to_query = self._lookup_queue.on_response(node_, response_msg.all_nodes, token, max_nodes)
        queries_to_send = self._get_lookup_queries(nodes_to_query)
    ...
    def on_timeout(self, node_):
        ...
        queries_to_send = self._get_lookup_queries(nodes_to_query)
        lookup_done = not self._num_parallel_queries
        return (queries_to_send, self._num_parallel_queries, lookup_done)
    ...
    def announce(self):
        ...
        for qnode in nodes_to_announce:
            ...
            msg = message.OutgoingAnnouncePeerQuery(self._my_id, self._info_hash, self._bt_port, qnode.token)
...
class LookupManager(object):
    ...
    def get_peers(self, lookup_id, info_hash, callback_f, bt_port = 0):
        lookup_q = GetPeersLookup(self.my_id, lookup_id, info_hash, callback_f, bt_port)
        return lookup_q

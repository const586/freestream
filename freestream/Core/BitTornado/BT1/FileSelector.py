...
class FileSelector:
    ...
    def __init__(self, files, piece_length, storage, storagewrapper, sched, failfunc):
        self.files = files
    ...
    def unpickle(self, d):
        ...
        if not pieces:
            return
        if DEBUG:
            log('fileselector::unpickle: pieces', pieces)
    ...
    def _set_files_disabled(self, old_priority, new_priority):
        if DEBUG:
            print >> sys.stderr, 'FileSelector:_set_files_disabled'
            print >> sys.stderr, 'old_priority', old_priority
            print >> sys.stderr, 'new_priority', new_priority
        ...
        if DEBUG:
            print >> sys.stderr, 'FileSelector:_set_files_disabled'
            print >> sys.stderr, 'data_to_update', data_to_update
        for piece, start, length in data_to_update:
            if self.storagewrapper.has_data(piece):
                data = self.storagewrapper.read_raw(piece, piece, start, length)
                if data is None:
                    if DEBUG:
                        print >> sys.stderr, 'FileSelector:_set_files_disabled: cannot read piece', piece
                    return False
        ...
        for piece, start, data in buffer:
            if not self.storagewrapper.write_raw(piece, piece, start, data):
                if DEBUG:
                    print >> sys.stderr, 'FileSelector:_set_files_disabled: cannot write piece', piece
                return False
            data.release()
        ...
        if DEBUG:
            print >> sys.stderr, 'FileSelector:_set_files_disabled'
            print >> sys.stderr, 'changed_pieces', changed_pieces
    ...
    def _set_piece_priority(self, new_priority):
        ...
        if DEBUG:
            log('FileSelector::_set_piece_priority: new_piece_priority', new_piece_priority, 'new_blocked', new_blocked, 'new_unblocked', new_unblocked)
        if new_blocked:
    ...
    def set_priorities_now(self, new_priority = None):
        if not new_priority:
            ...
            if not new_priority:
                return
        if DEBUG:
            log('FileSelector::set_priorities_now: new_priority', new_priority)
            print_stack()
        old_priority = self.priority
        self.priority = new_priority
        if not self._set_files_disabled(old_priority, new_priority):
            if DEBUG:
                log('FileSelector:set_priorities_now: _set_files_disabled returned false')
            return
        self.piece_priority = self._set_piece_priority(new_priority)

    def set_priorities(self, new_priority):
        if DEBUG:
            log('FileSelector::set_priorities: new_priority', new_priority)
            print_stack()
        self.new_priority = new_priority

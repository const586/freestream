...
class TimeoutFinder:
    ...
    def ping(self):
        ...
        if self.initial_ping:
            sock.sendto(struct.pack('!Id', 0, float(TimeoutFinder.PINGBACK_TIMES[self.timeout_index])), TimeoutFinder.PINGBACK_ADDRESS)
        else:
            sock.sendto(struct.pack('!Id', TimeoutFinder.PINGBACK_TIMES[self.timeout_index], time.time()), TimeoutFinder.PINGBACK_ADDRESS)
...
if __name__ == '__main__':
    ...
    rawserver_ = RawServer.RawServer(Event(), 60.0, 300.0, False, failfunc=fail, errorfunc=error)

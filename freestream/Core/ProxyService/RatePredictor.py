...
class RatePredictor:
    ...
    def __init__(self, raw_server, rate_measure, max_rate, probing_period = 2):
        ...
        self.probing_period = probing_period
...
class ExpSmoothRatePredictor(RatePredictor):
    ...
    def __init__(self, raw_server, rate_measure, max_rate, alpha = 0.5, max_period = 30, probing_period = 2):
        ...
        if DEBUG:
            print >> sys.stderr, 'RatePredictor:__init__'
        self.alpha = alpha
    ...
    def update(self):
        if DEBUG:
            print >> sys.stderr, 'RatePredictor:update'
        self.raw_server.add_task(self.update, self.probing_period)
        current_value = self.rate_measure.get_rate() / 1000.0
        current_time = clock()
    ...
    def predict(self):
        if DEBUG:
            print >> sys.stderr, 'RatePredictor:predict'
    ...
    def has_capacity(self):
        if DEBUG:
            print >> sys.stderr, 'RatePredictor:has_capacity'
        result = None
        if self.value is None:
            result = True
        else:
            result = 1.0 - float(self.value) / self.max_rate > MIN_CAPACITY
        return result

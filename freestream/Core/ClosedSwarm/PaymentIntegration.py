...
class PaymentSystem:
    ...
    def request_code(self):
        ...
        data = urllib.urlencode({'mobile': self.mobile_number,
         'request': 'code',
        ...
        s = f.read()
        f.close()
        p = re.compile('error=(\\S+)', re.MULTILINE)
        m = p.search(s)
        error = m.group(1)
    ...
    def verify_code(self, code):
        import sys
        print >> sys.stderr, {'request': 'validate',
         'code': code,
         'mobile': self.mobile_number,
         'perm_id': self.perm_id,
         'swarm_id': self.swarm_id}
        data = urllib.urlencode({'request': 'validate',
         'code': code,
         'mobile': self.mobile_number,
         'perm_id': self.perm_id,
         'swarm_id': self.swarm_id})
        ...
        s = f.read()
        f.close()
        p = re.compile('code=(\\w+)', re.MULTILINE)
        m = p.search(s)
        if m != None:
            validation = m.group(1)
        else:
            validation = None
        p = re.compile('poa=(.*)..error=', re.DOTALL)
        m = p.search(s)
        if m != None:
            poa = m.group(1)
        else:
...
def wx_get_poa(url, swarm_id, perm_id, root_window = None, swarm_title = 'Unknown content'):
    d = PaymentDialog(swarm_title, url, swarm_id, perm_id)
    retval = d.ShowModal()
    try:
        poa_b64 = d.get_poa()
...
if __name__ == '__main__':
    ...
    t.start()
    d = PaymentDialog('Test file', 'http://127.0.0.1:9090', '1234', 'myid')
    retval = d.ShowModal()

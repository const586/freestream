...
OLPROTO_VER_FIRST = 1
OLPROTO_VER_SECOND = 2
OLPROTO_VER_THIRD = 3
OLPROTO_VER_FOURTH = 4
OLPROTO_VER_FIFTH = 5
OLPROTO_VER_SIXTH = 6
OLPROTO_VER_SEVENTH = 7
OLPROTO_VER_EIGHTH = 8
OLPROTO_VER_NINETH = 9
OLPROTO_VER_TENTH = 10
OLPROTO_VER_ELEVENTH = 11
OLPROTO_VER_TWELFTH = 12
OLPROTO_VER_THIRTEENTH = 13
OLPROTO_VER_FOURTEENTH = 14
OLPROTO_VER_FIFTEENTH = 15
...
class SecureOverlay():
    ...
    def register(self, launchmanycore, max_len):
        ...
        self.overlay_rawserver = self.multihandler.newRawServer(overlay_infohash, self.rawserver.doneflag, protocol_name)
        ...
        try:
            ...
            if oc is None:
                if dns is None:
                    ...
                    self._connect_dns(dns, lambda exc, dns2, peerpermid, selver: self._whoishe_callback(exc, dns2, peerpermid, selver, expectedpermid, callback))
            else:
        ...
        try:
            if exc is None:
                if peerpermid == expectedpermid:
                    ...
                    callback(KeyError('Recorded IP address + port now of other permid'), dns, expectedpermid, 0)
            else:
    ...
    def externally_handshaked_connection_made(self, singsock, options, msg_remainder):
        if DEBUG:
            print >> sys.stderr, 'secover: externally_handshaked_connection_made', singsock.get_ip(), singsock.get_port()
    ...
    def start_connection(self, dns):
        ...
        oc = OverlayConnection(self, singsock, self.rawserver, locally_initiated=True, specified_dns=dns)
        singsock.set_handler(oc)
        return oc
    ...
    def cleanup_admin_and_callbacks(self, oc, exc):
        ...
        if oc.is_auth_done() and self.userconnhandler is not None:
            self.userconnhandler(exc, oc.get_auth_permid(), oc.get_sel_proto_ver(), oc.is_locally_initiated(), None)
...
def is_proto_version_supported(low_ver, cur_ver):
    if cur_ver != OLPROTO_VER_CURRENT:
        if low_ver > OLPROTO_VER_CURRENT:
            return False
        if cur_ver < OLPROTO_VER_LOWEST:
            return False
        if cur_ver < OLPROTO_VER_CURRENT and cur_ver not in SupportedVersions:
            return False
    return True
...
def select_supported_protoversion(his_low_ver, his_cur_ver):
    ...
    if his_cur_ver != OLPROTO_VER_CURRENT:
        if his_low_ver > OLPROTO_VER_CURRENT:
            return selected
        if his_cur_ver < OLPROTO_VER_LOWEST:
            return selected
        if his_cur_ver < OLPROTO_VER_CURRENT and his_cur_ver not in SupportedVersions:
            return selected

...
class SearchPathMapper(AbstractPathMapper):
    ...
    def process_search_p2p(self, searchstr):
        ...
        if False:
            ...
            streaminfo = {'statuscode': 301,
             'statusmsg': nextlinkpath}
            return streaminfo
    ...
    def sesscb_got_remote_hits(self, id, permid, query, remotehits):
        try:
            et = time.time()
            diff = et - self.st
            print >> sys.stderr, 'bg: search: Got', len(remotehits), 'remote hits'
...
class Hits2AnyPathMapper(AbstractPathMapper):
    ...
    def get_torrentstreaminfo(self, infohash, hit):
        ...
        try:
            if hit['hittype'] == 'localdb':
                ...
                torrentstreaminfo = {'statuscode': 200,
                 'mimetype': TSTREAM_MIME_TYPE,
                 'stream': stream,
                 'length': length}
    ...
    def create_thumbstreaminfo(self, thumbtype, thumbdata):
        if thumbtype is None:
            ...
            thumbstreaminfo = {'statuscode': 200,
             'mimetype': thumbtype,
             'stream': stream,
             'length': length}
    ...
    def get_nsmetastreaminfo(self, infohash, hit, hiturlpathprefix, hitpath):
        ...
        nsmetastreaminfo = {'statuscode': 200,
         'mimetype': 'text/xml',
         'stream': nsmetastream,
         'length': len(nsmetarepr)}
...
def nsmetahit2nsmetarepr(hit, hitpath):
    ...
    livetimepoint = now2formatRFC3339()
    s = ''
...
def hack_make_default_merkletorrent(title):
    ...
    info = {}
    info['name'] = title
    info['length'] = 1073741824
    info['piece length'] = 65536
    info['root hash'] = '********************'
    metainfo['info'] = info
    mdict = {}

...
class XXXFilter:
    ...
    def isXXXTorrent(self, files_list, torrent_name, tracker, comment = None):
        ...
        is_xxx = len(filter(self.isXXX, terms)) > 0 or self.isXXX(torrent_name, False) or self.isXXX(tracker, False) or comment and self.isXXX(comment, False)
        if DEBUG:
            if is_xxx:
    ...
    def isXXX(self, s, isFilename = True):
        ...
        if self.isXXXTerm(s):
            return True
        elif not self.isAudio(s) and self.foundXXXTerm(s):
            return True
